...
app.get('/getEnv', function (req, res) {
  try {
    if (process.env.DEMO_APP_CLIENT_ID == undefined || process.env.DEMO_APP_CLIENT_ID == null) {
      res.status(500).send({
...
app.post('/getPersonData', function (req, res, next) {
  ...
  try {
    ...
    var myinfoConfig = config.MYINFO_CONNECTOR_CONFIG(process.env.DEMO_APP_CLIENT_ID,process.env.DEMO_APP_CLIENT_SECRET);
